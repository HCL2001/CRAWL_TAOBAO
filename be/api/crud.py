        ...
        for script_tag in script_tags:
            ...
            if script_content and "g_page_config" in script_content:
                ...
                try:
                    ...
                    for item in data:
                        name = translator.translate(item['raw_title'], src=constant.CHINESE , dest=constant.VIETNAMESE)
                        # print(item)
                        objectDto = {
                            'id': counter,
                            'name': name.text,
                    ...
                    await save_list_tabao_to_database(product_list, database.SQLALCHEMY_DATABASE_URL)
                except json.JSONDecodeError as e:
                    print(e)
    return product_list

async def save_list_tabao_to_database(data_list, db_url):
    try:
        ...
        for item in data_list:
            taobao_item = models.Taobao(
                name=item['name'],
                price=item['price'],
                shop_name=item['shopName'],
                is_deleted=item['isDeleted'],
            )
            session.add(taobao_item)
        session.commit()
        session.close()

        print("List data has been saved to the database successfully.")
